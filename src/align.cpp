                 ...
                 )
{
  ...
  Backtrack<Tuint> mB = opt.backtracking ? Backtrack<Tuint>(n, t) : Backtrack<Tuint>();
                       ...
                       )
{
  ...
  for (long i = 0; i < n; ++i)
  {
    ...
    {
      // Check for substitutions and if it has a higher score than the insertion
      vF[v] = simdpp::max(vF[v], static_cast<Tpack>(vH_up[v] - gap_open_pack));
      vH[v] = simdpp::max(static_cast<Tpack>(vH_up[v - 1] + vW[v - 1]), vF[v]);

      // Deletions pass 1
      vE[v] = simdpp::max(static_cast<Tpack>(vH[v - 1] - gap_open_pack), vE[v - 1]);
    } /// Done calculating vectors v=1,...,t-1
