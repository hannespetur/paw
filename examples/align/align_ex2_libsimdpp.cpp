...
main(int, char **)
{
  ...
  auto t0 = Ttime::now();
  paw::AlignmentResults<uint16_t> ar = paw::arch_null::global_alignment(database, query, opts);
  auto t1 = Ttime::now();
  std::cout << "null " << Tduration(t1 - t0).count() << " ms\n";
  std::cout << "score = " << ar.score << "\n";

  t0 = Ttime::now();
  ar = paw::arch_sse2::global_alignment(database, query, opts);
  t1 = Ttime::now();
  std::cout << "sse2 " << Tduration(t1 - t0).count() << " ms\n";
  std::cout << "score = " << ar.score << "\n";

  t0 = Ttime::now();
  ar = paw::arch_sse4p1::global_alignment(database, query, opts);
  t1 = Ttime::now();
  std::cout << "sse4_1 " << Tduration(t1 - t0).count() << " ms\n";
  std::cout << "score = " << ar.score << "\n";

  t0 = Ttime::now();
  ar = paw::global_alignment(database, query, opts);
  t1 = Ttime::now();
