...
#include <paw/station/algorithm.hpp> // stations::count
...
{
  T ints;
  REQUIRE(paw::count_if(ints.begin(), ints.end(), [](int i){
    return i <= 5;
  }) == 0);
}
...
{
  T ints =
  {0, 2, 4, 6, 0, 78, 8, 1, 2, 4, 6, 5, 7, 8, 554, 345, 23, 53, 64, 0, 65, 13123, 4444, 0};
  REQUIRE(paw::count_if(ints.begin(), ints.end(), [](int i){
    return i <= 5;
  }) == 10);
}
...
check_int_count_if_case2()
{
  ...
  ints.resize(1000000, 0); // 1000000 - 5 = 9999995 less or equal to 5
  REQUIRE(paw::count_if(ints.begin(), ints.end(), [](int i){
    return i <= 5;
  }) == 999995);
}
