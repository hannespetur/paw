...
#include <paw/station/partition_iterator.hpp>
...
TEST_CASE("Get split iterators for an empty vector")
{
  ...
  SECTION("One part")
  {
    std::vector<std::vector<double>::iterator> split_iters = paw::get_partition_iterators(
      doubles.begin(),
      doubles.end(),
      1 /*PARTS*/);
...
TEST_CASE("Get split iterators for some vector")
{
  ...
  SECTION("One part")
  {
    std::vector<std::vector<int>::iterator> split_iters = paw::get_partition_iterators(
      ints.begin(),
      ints.end(),
      1 /*PARTS*/);
    REQUIRE(split_iters.size() == 2);
